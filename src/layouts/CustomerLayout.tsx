...
const CustomerLayout: React.FC = () => {
    ...
    useEffect(() => {
        if (debouncedSearchQuery.trim().length > 1) {
            ...
            axiosClient.get(`/api/_search/products?query=${encodeURIComponent(debouncedSearchQuery)}`)
                .then(response => {
            ...
            <header className="bg-white/95 backdrop-blur-sm shadow-md sticky top-0 z-50">
                <nav className="container mx-auto px-6 py-4 flex justify-between items-center">
                    ...
                    <div ref={searchRef} className="relative w-full max-w-md">
                        <form onSubmit={handleSearchSubmit}>
                            ...
                            <input
                                ...
                                className="w-full pl-12 pr-4 py-2.5 border border-gray-200 bg-gray-50 rounded-full text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent transition"
                            />
                        </form>
                        ...
                        {isSearchFocused && (searchQuery.length > 1) && (
                            <div className="absolute top-full mt-2 w-full bg-white rounded-lg shadow-2xl border overflow-hidden z-10">
                                ...
                                {!isSearchLoading && searchResults.length === 0 && debouncedSearchQuery.length > 1 && (
                                    ...
                                    <ul>
                                        {searchResults.map(product => (
                                            <li key={product.id} onClick={handleResultClick}>
                                                <Link to={`/products/${product.slug}`} className="flex items-center p-3 hover:bg-gray-100 transition-colors">
                                                    <img 
                                                        src={`https://placehold.co/60x60/EFEFEF/333333?text=${encodeURIComponent(product.name.split(' ')[0])}`} 
                                                        alt={product.name} 
                                                        className="w-12 h-12 object-cover rounded-md"
                                                    />
                                                    <div className="ml-4">
                                                        <p className="font-semibold text-gray-800">{product.name}</p>
                                                        <p className="text-sm text-indigo-600">{product.variants?.[0]?.price.toLocaleString('vi-VN')} VND</p>
                                                    </div>
                                                </Link>
                                            </li>
                                        ))}
                                        <li onClick={handleResultClick}>
                                            <Link to={`/search?query=${encodeURIComponent(searchQuery.trim())}`} className="block text-center p-3 bg-gray-50 hover:bg-gray-200 text-sm font-semibold text-indigo-600 transition-colors">
                                                Xem tất cả kết quả
                                            </Link>
                                        </li>
                                    </ul>
                    ...
                    <div className="flex items-center space-x-4">
                        ...
                        {isAuthenticated() ? (
                            <>
                                <NavLink to="/account/profile" className="text-gray-600 hover:text-indigo-600 font-semibold transition-colors duration-300">
                                    Tài khoản
                                </NavLink>
                                <button
