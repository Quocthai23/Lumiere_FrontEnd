...
async function fetchGroupsSnapshot(productId: number): Promise<{
  groupsSnap: OptionGroupDTO[];
  selectsSnap: Record<number, OptionSelectDTO[]>;
}> {
  const groupsSnap = (await httpClient.get<OptionGroupDTO[]>(`/option-groups?productId=${productId}`)) || [];
  const selectsSnap: Record<number, OptionSelectDTO[]> = {};
  await Promise.all(groupsSnap.map(async g => {
...
export default function OptionGroupManager({ productId }: { productId: number }) {
  ...
  const autoSplitFromVariants = async () => {
    try {
      ...
      // 2) parse attributes from variant name: "Product - A / B / C" -> ['A','B','C']
      const rows: string[][] = [];
      for (const v of variants) {
        const nm = (v.name || '').trim();
        const after = nm.includes(' - ') ? nm.split(' - ').slice(1).join(' - ') : nm;
        const attrs = after.split('/').map(s => s.trim()).filter(Boolean);
        if (attrs.length > 0) rows.push(attrs);
      }
      if (!rows.length) {
        alert('Không phân tích được thuộc tính từ tên biến thể.');
        return;
      }
      ...
      for (let i = 0; i < need; i++) {
        const idx = sortedGroups.length + i + 1;
        const g = await httpClient.post<OptionGroupDTO>('/option-groups', {
          productId,
          name: `Thuộc tính ${idx}`,
          code: `ATTR_${idx}`,
          position: idx
      ...
      for (const r of rows) {
        for (let c = 0; c < maxCols; c++) {
          const val = (r[c] || '').trim();
          if (!val) continue;
          uniqueByCol[c] = uniqueByCol[c] || new Set<string>();
          uniqueByCol[c].add(val);
        }
      }

      // 5) bulk insert missing selects for each group/column
      const bulkItems: OptionSelectDTO[] = [];
      for (let c = 0; c < maxCols; c++) {
        const g = allGroups[c];
        if (!g) continue;
        const existing = (selects[g.id!] || []);
        const existLower = new Set(existing.map(s => s.name.trim().toLowerCase()));
        let basePos = existing.length;

        for (const name of Array.from(uniqueByCol[c] || [])) {
          if (existLower.has(name.trim().toLowerCase())) continue;
          bulkItems.push({
            optionGroupId: g.id!,
            name,
            code: toCodeFromName(name),
      ...
      if (bulkItems.length) {
        await httpClient.post<OptionSelectDTO[]>('/option-selects/bulk', bulkItems);
      }

      // 6) refresh UI + đồng bộ biến thể theo group/select mới nhất
      await fetchGroups(); // cập nhật UI

      // Dùng snapshot mới nhất từ server để build payload an toàn
      ...
      if (!payload.length) {
        alert('Đã chia phân loại. Chưa có lựa chọn nào đang bật để đồng bộ biến thể.');
        return;
      }
